...
- Store Kubernetes resources in `k8s/base` and a `k8s/overlays/starmoon` overlay using Kustomize.
...
- `kubectl apply -k k8s/overlays/starmoon` produces valid resources.
